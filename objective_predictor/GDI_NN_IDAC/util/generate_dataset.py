...
class solvent_dataset_binary:
    ...
    def __init__(self, input_file_path = "./data/output_binary_with_inf_all.csv",
    ...
    def __getitem__(self,idx):
        ...
        sample["solv1_id"] = ids[0]
        sample["solv2_id"] = ids[1]
        return sample
    ...
    def generate_solvsys(self,batch_size=5):
        ...
        return solvsys
    def predict(self,idx,model,empty_solvsys=None,gamma_grad=False):
        ...
        if empty_solvsys:
            if gamma_grad:
                with torch.backends.cudnn.flags(enabled=False):
                    predict, y1_x1, y2_x1 = model(sample,empty_solvsys, gamma_grad)
                    predict =  predict.detach().cpu().numpy().squeeze()
                    y1_x1 = y1_x1.detach().cpu().numpy().squeeze()
                    y2_x1 = y2_x1.detach().cpu().numpy().squeeze()
                return true, predict, {"y1_x1": y1_x1, "y2_x1": y2_x1}
            else:
                predict = model(sample,empty_solvsys, gamma_grad).detach().cpu().numpy().squeeze()
        else:
            if gamma_grad: 
                with torch.backends.cudnn.flags(enabled=False):
                    predict, y1_x1, y2_x1 = model(sample)
                    predict =  predict.detach().cpu().numpy().squeeze()
                    y1_x1 = y1_x1.detach().cpu().numpy().squeeze()
                    y2_x1 = y2_x1.detach().cpu().numpy().squeeze()
                return true, predict, {"y1_x1": y1_x1, "y2_x1": y2_x1}
    ...
    def predict_new(self,model,chemical_list,smiles_list,solv1_x,gamma_grad=False, gamma_list=None, solv_ids=None):
        empty_solvsys = self.generate_solvsys(batch_size=1).to("cuda")
        sample = self.generate_sample(chemical_list,smiles_list,solv1_x,gamma_list)
        sample['solv1_x'] = torch.tensor(sample['solv1_x']).unsqueeze(0).cuda()
        sample['intra_hb1'] = torch.tensor(sample['intra_hb1']).unsqueeze(0).cuda()
        sample['intra_hb2'] = torch.tensor(sample['intra_hb2']).unsqueeze(0).cuda()
        sample['inter_hb'] = torch.tensor(sample['inter_hb']).unsqueeze(0).cuda()
        if not (solv_ids is None):
            sample["solv1_id"] = torch.tensor([int(solv_ids[0].split("_")[1])]).cuda()
            sample["solv2_id"] = torch.tensor([int(solv_ids[1].split("_")[1])]).cuda()
            #print(1)
        #print(sample)
        if gamma_grad:
            with torch.backends.cudnn.flags(enabled=False):
                ...
                y1_x1 = y1_x1.detach().cpu().numpy().squeeze()
                y2_x1 = y2_x1.detach().cpu().numpy().squeeze()
            return predict, {"y1_x1": y1_x1, "y2_x1": y2_x1}
        predict = model(sample,empty_solvsys, gamma_grad).detach().cpu().numpy().squeeze()
        return predict
    def predict_new_with_uncertainty(self,model,cp_list,chemical_list,smiles_list,solv1_x,gamma_list=None,gamma_grad=False,solv_ids=None):
        pred_all = []
        y1_x1_all = []
        y2_x1_all = []
        for checkpoint in cp_list:
            ...
            if gamma_grad:
                predict, grad_dict = self.predict_new(model=model,chemical_list=chemical_list,smiles_list=smiles_list,solv1_x=solv1_x,gamma_grad=gamma_grad,gamma_list=gamma_list,solv_ids=solv_ids)
                y1_x1_all.append(grad_dict["y1_x1"])
                y2_x1_all.append(grad_dict["y2_x1"])
            else:
                predict = self.predict_new(model=model,chemical_list=chemical_list,smiles_list=smiles_list,solv1_x=solv1_x,gamma_grad=gamma_grad,gamma_list=gamma_list,solv_ids=solv_ids)
            pred_all.append(predict)
        pred_all_mean = np.mean(np.array(pred_all),axis = 0)
        pred_all_std = np.std(np.array(pred_all),axis=0) 
        if gamma_grad:
            y1_x1_all_mean = np.mean(np.array(y1_x1_all),axis = 0)
            y1_x1_all_std = np.std(np.array(y1_x1_all),axis=0) 
            y2_x1_all_mean = np.mean(np.array(y2_x1_all),axis = 0)
            y2_x1_all_std = np.std(np.array(y2_x1_all),axis=0) 
            grad_dict = {
                "y1_x1": y1_x1_all_mean,
                "y1_x1_std": y1_x1_all_std,
                "y2_x1": y2_x1_all_mean,
                "y2_x1_std": y2_x1_all_std,
            }
            return pred_all_mean, pred_all_std, grad_dict
        else:
            return pred_all_mean, pred_all_std 
    def get_VLE(self,model,chemical_list,smiles_list,solv1_x_range,temperature=298.,cp_list=None,uncertainty=False,solv_ids=None):
        solv1_gam = []
        solv2_gam = []
        ...
        if uncertainty:
            ...
            if cp_list:               
                for solv1_x in solv1_x_range:
                    pred,std = self.predict_new_with_uncertainty(model,cp_list,chemical_list,smiles_list,solv1_x,gamma_list=None,solv_ids=solv_ids)
    ...
    def plot_VLE(self,ax, model,chemical_list,smiles_list,solv1_x_range,temperature=298.,
                 ...
                 overlay_with_cosmo=False,overlay_with_aspen_and_cosmo=False, solv_ids=None):
        if convert_cosmo:
        ...
        plot_data = self.get_VLE(model,chemical_list,smiles_list,solv1_x_range,temperature=temperature,
                                 cp_list=cp_list,uncertainty=uncertainty,solv_ids=solv_ids)
        ...
        if overlay_with_aspen:
            ...
            ax.plot(plot_data["x1"],plot_data["pressure"],label="$liquid$",color='#00549F',marker='s',markersize=10, linewidth=4) # "#00549F", "#CC071E",
            ax.plot(plot_data["y1"],plot_data["pressure"],label="$vapor$",color='#CC071E',marker='o',markersize=10, linewidth=4)
        ax.set_xticks(np.arange(0,1.2,0.2))
        ax.set_xlabel("$x_1$, $y_1$")
        ax.set_ylabel("Pressure (bar)")
        #ax.set_title(f"Molecule 1: {chemical_list[0]}\nMolecule 2: {chemical_list[1]}") #("$P_{xy}$"+" for {}(1) and {}(2)".format(chemical_list[0],chemical_list[1]))
        #ax.legend(loc='best')
        plt.tight_layout()
        if save_figure:
    ...
    def plot_gamma(self,chemical_list,plot_data,save_dir=None,save_figure=True,overlay_with_aspen=True,
                   overlay_with_cosmo=True,overlay_with_aspen_and_cosmo=True, uncertainty=True):
        fig,ax = plt.subplots(figsize=(3,2.8))
        ax.grid(color='lightgray',linewidth=0.75,alpha=0.5)
        if uncertainty == False: 
            plot_data["solv1_gam_std"] = 0
            plot_data["solv2_gam_std"] = 0
