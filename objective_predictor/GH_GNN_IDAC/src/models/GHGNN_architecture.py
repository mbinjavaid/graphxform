...
from .utilities.mol2graph import cat_dummy_graph


class MPNNconv(nn.Module):
    def __init__(self, node_in_feats, edge_in_feats, node_out_feats,
                 edge_hidden_feats=32, num_step_message_passing=1, device: torch.device = None):
    ...
    def forward(self, system_graph):
        ...
        for _ in range(self.num_step_message_passing):
            node_feats = F.relu(self.gnn_layer(x=node_feats.type(torch.FloatTensor).to(self.device),
                                               edge_index=edge_index.type(torch.LongTensor).to(self.device),
                                               edge_attr=edge_feats.type(torch.FloatTensor).to(self.device)))
...
class GHGNN(nn.Module):
    def __init__(self, v_in, e_in, u_in, hidden_dim, device: torch.device = None):
    ...
    def forward(self, solvent, solute, T, scaler=None, ln_gamma=True):
        ...
        if solvent.edge_attr.shape[0] == 0 or solute.edge_attr.shape[0] == 0:
            solvent = cat_dummy_graph(solvent, device=self.device)
            solute = cat_dummy_graph(solute, device=self.device)
            u1_dummy = torch.tensor([1,1,1]).reshape(1,-1)
            u2_dummy = torch.tensor([1,1,1]).reshape(1,-1)
            u1_dummy = u1_dummy.to(self.device)
            u2_dummy = u2_dummy.to(self.device)
